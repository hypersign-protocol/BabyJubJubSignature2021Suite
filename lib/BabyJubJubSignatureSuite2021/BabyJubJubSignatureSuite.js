...
class BabyJubJubSignature2021Suite extends LinkedDataSignature {
    constructor(options) {
        super({
            ...
            date: (0, utils_1.w3cDate)(options.date ? options.date : new Date()),
            key: options.key,
            proof: options.proof,
    ...
    createProof(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const merklized = yield this.canonize(options.document);
            const verifyData = (yield merklized.root()).bigInt();
            const signature = yield this.sign({
                verifyData: verifyData,
                proof: proof,
            });
            proof[this.proofSignatureKey] = this.convertMultiBase(this.compactSignature(signature));
            return proof;
        });
    }
    sign(params) {
